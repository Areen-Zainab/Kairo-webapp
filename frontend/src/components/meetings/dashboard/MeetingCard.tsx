...
const MeetingCard: React.FC<{ meeting: Meeting }> = ({ meeting }) => {
  ...
  const handleCardClick = () => {
    if (meeting.status === 'live') {
      navigate(`/workspace/meetings/live/${meeting.id}`);
    } else if (meeting.status === 'upcoming') {
      navigate(`/workspace/meetings/pre/${meeting.id}`);
  ...
  return (
    <div 
      className="group relative rounded-lg p-5 transition-all duration-200 cursor-pointer hover:shadow-lg bg-white border border-gray-200 hover:border-purple-300 hover:shadow-purple-500/5 dark:bg-slate-800/40 dark:border-slate-700/50 dark:hover:border-purple-500/50"
    ...
    >
      ...
      <div className="flex items-center space-x-2 pt-3 border-t border-gray-200 dark:border-slate-700/50">
        ...
        {meeting.status === 'completed' && (
          <button 
            className="flex-1 flex items-center justify-center space-x-1.5 px-3 py-2 rounded-md text-sm font-medium transition-all bg-gray-100 hover:bg-gray-200 border border-gray-300 text-gray-700 dark:bg-slate-700/30 dark:hover:bg-slate-700/50 dark:border-slate-600/50 dark:text-slate-300"
            onClick={(e) => {
              e.stopPropagation();
              navigate(`/workspace/meetings/${meeting.id}`);
            }}
        ...
        <button 
          className="p-2 rounded-md transition-all bg-gray-100 hover:bg-gray-200 border border-gray-300 text-gray-600 hover:text-gray-900 dark:bg-slate-700/30 dark:hover:bg-slate-700/50 dark:border-slate-600/50 dark:text-slate-400 dark:hover:text-white"
